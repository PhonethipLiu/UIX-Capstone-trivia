...
getTrivia().then((resolve) => {
    ...
    buildResultObj(saveResult.gameName, saveResult.gameResult);
...
function buildResultObj(resolve){
        console.log("DOM.JS LINE 210: what is resultData?", resolve);
    $("#user-game-result").append(`<h5>${resolve.gameName}</h5> <p>${resolve.gameResult}</p> <div class="result-footer">
    <button class="btn btn-secondary btn-sm"  id="quiz-delete-result">Delete</button>
    </div>
</div>`);
  }
